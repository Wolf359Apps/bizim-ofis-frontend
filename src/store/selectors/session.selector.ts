...
export const navMenuItems = createSelector(
	...
	(roles, isLoggedIn, translations): MenuItem[] => {
		...
		if (isLoggedIn)
    		...
    		if (roles.includes(AuthRole.MUSTERI_VIEW))
			items.push(navItems.miMusteriler);

		if (roles.includes(AuthRole.SAAT_VIEW))
			items.push(navItems.miSaatler);

		if (roles.includes(AuthRole.MASRAF_VIEW))
			items.push(navItems.miMasraflar);
